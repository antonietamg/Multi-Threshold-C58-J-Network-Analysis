...
# - Louvain communities (incluye dendrograma)
...
# 2.4) Comunidades: Louvain
detect_louvain <- function(g, resolution = 1, seed = 1) {
  if (ecount(g) == 0) {
  ...
  # En igraph, Louvain procesa nodos en orden aleatorio
  set.seed(seed)
  ...
  list(
    ...
    membership = memb,
    modularity = mod,
    hierarchical = igraph::is_hierarchical(comm),
    algorithm = igraph::algorithm(comm)
  )
}
...
# 3.2) Dendrograma / árbol de partición:
# - si el objeto de comunidades es jerárquico -> usa dendrograma real
# - si NO lo es (caso típico al cambiar de método) -> dibuja árbol de partición final
plot_cluster_dendrogram <- function(comm_obj, membership_vec, node_info_df,
  ...
  # ------------------------------------------------------------------
  # A) Si hay estructura jerárquica real -> usa tu dendrograma original
  # ------------------------------------------------------------------
  if (!is.null(comm_obj) && igraph::is_hierarchical(comm_obj)) {
    ...
    return(
      ggplot() +
        ...
        theme(
          plot.title = element_text(face = "bold", hjust = 0, size = 18),
          plot.margin = margin(10, 260, 10, 10)
        )
    )
  }
  
  # ------------------------------------------------------------------
  # B) Fallback para Louvain: árbol de partición final (2 niveles)
  #    Mantiene estética similar aunque NO es un dendrograma "nativo"
  # ------------------------------------------------------------------
  lab_h <- tibble(node = names(memb)) %>%
    dplyr::left_join(node_map, by = c("node" = "node")) %>%
    dplyr::mutate(
      cluster   = factor(memb[node]),
      structure = factor(structure, levels = names(structure_palette)),
    ...
    dplyr::arrange(cluster, structure, label) %>%
  ...
  x_label       <- -label_x_mult   * x_root
  x_cluster_txt <- -cluster_x_mult * x_root
  
  seg_leaf <- lab_h %>%
    dplyr::transmute(
      x = x_leaf, y = y,
      xend = x_cluster, yend = y
    )
  
  seg_cluster <- cl_df %>%
    dplyr::transmute(
      x = x_cluster, y = ymin,
      xend = x_cluster, yend = ymax
    )
  
  seg_root_branches <- cl_df %>%
    dplyr::transmute(
      x = x_cluster, y = y,
      xend = x_root, yend = y
    )
  
  seg_root <- if (nrow(cl_df) >= 2) {
    tibble(
      x = x_root,
      y = min(cl_df$y),
      xend = x_root,
  ...
  seg_all <- bind_rows(seg_leaf, seg_cluster, seg_root_branches, seg_root)
  ...
  ggplot() +
    ...
    scale_colour_manual(values = structure_palette, guide = "none") +
    scale_x_reverse(expand = expansion(mult = c(0.02, 1.20))) +
    scale_y_reverse(expand = expansion(mult = c(0.02, 0.02))) +
    coord_cartesian(clip = "off") +
    labs(title = title) +
    theme_void(base_size = 16) +
...
# 4.3) Comunidades Louvain
# resolution = 1 -> modularidad estándar
comm_list <- list(
  "C57BL/6" = detect_louvain(graph_list[["C57BL/6"]], resolution = 1, seed = 1),
  "C58/J"   = detect_louvain(graph_list[["C58/J"]],   resolution = 1, seed = 2)
)
...
p_a <- plot_cluster_network(
  ...
  title = "C57BL/6 Clustering network (Louvain)",
  seed = 1,
  show_structure_legend = FALSE
)
...
p_b <- plot_cluster_network(
  ...
  structure_palette = structure_palette,
  title = "C58/J Clustering network (Louvain)",
  seed = 2,
  show_structure_legend = FALSE
)

# # 4.5) Paneles c/d: dendrograma real si existe; si no, árbol de partición final
# p_c <- plot_cluster_dendrogram(
#   comm_list[["C57BL/6"]]$comm,
#   comm_list[["C57BL/6"]]$membership,
#   node_info_df = node_info,
#   structure_palette = structure_palette,
#   title = "Cluster composition in C57BL/6 (Louvain)",
#   label_size = 4.2,
#   cluster_num_size = 6.5
# )
...
#   node_info_df = node_info,
#   structure_palette = structure_palette,
#   title = "Cluster composition in C58/J (Louvain)",
#   label_size = 4.2,
#   cluster_num_size = 6.5
# )
# 
...
# write.csv(deg_df, file = "degree_distrib_louvain.csv")
# 
